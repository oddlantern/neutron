import type { ResolvedExtensionField, ValidatedTokens } from "../design/types.js";
 ...
 */
function lerpExpr(fieldName: string, field: ResolvedExtensionField): string {
  switch (field.type) {
    case "color":
    case "color-static":
 ...
 */
function dartType(field: ResolvedExtensionField): string {
  switch (field.type) {
    case "color":
    case "color-static":
      return "Color";
...
function dartLightValue(field: ResolvedExtensionField): string {
  switch (field.type) {
    case "color":
      return hexToDartColor(field.value.light);
    case "color-static":
      return hexToDartColor(field.value);
    case "number":
      return `${field.value}`;
    case "number-themed":
      return `${field.value.light}`;
    case "string":
      return `'${field.value}'`;
  }
}
...
function dartDarkValue(field: ResolvedExtensionField): string {
  switch (field.type) {
    case "color":
      return hexToDartColor(field.value.dark);
    case "color-static":
      return hexToDartColor(field.value);
    case "number":
      return `${field.value}`;
    case "number-themed":
      return `${field.value.dark}`;
    case "string":
      return `'${field.value}'`;
  }
}
...
export function generateThemeExtensions(tokens: ValidatedTokens): string {
  ...
  for (const ext of extensions) {
    ...
    for (const field of fields) {
      lines.push(`    ${dartType(field)}? ${field.name},`);
    }
    lines.push("  }) {");
    ...
    lines.push(`    return ${ext.name}(`);
    for (const field of fields) {
      lines.push(`      ${field.name}: ${lerpExpr(field.name, field)},`);
    }
    lines.push("    );");
    lines.push("  }");
    lines.push("}");
