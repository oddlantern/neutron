...
Cross-ecosystem monorepo workspace tool. Package: `@oddlantern/neutron`. Binary: `neutron`.
...
src/
  ...
  lock.ts                # neutron.lock read/write/merge
  ...
  version.ts             # Package version + NEUTRON_ROOT path resolution
  config/
    defaults.ts          # Default config values (lint categories, format options, ignore patterns)
    schema.ts            # Zod schema for neutron.yml (ecosystem-centric format/lint/commits)
  ...
  commands/
    add.ts               # Scaffold new package + update neutron.yml
    ...
    init.ts              # Scan repo, generate neutron.yml interactively
    install.ts           # Write git hooks to .git/hooks/
    lint.ts              # Run linters across all packages per ecosystem
    migrate.ts           # Migrate external tool configs into neutron.yml
    ...
    reconcile.ts         # Reconciliation mode when neutron.yml already exists
...
- **`neutron.lock` is the version policy.** When it exists, `neutron check` validates manifests against the lock, not just against each other. `neutron upgrade` updates manifests and the lock atomically.
- **Progressive dependency analysis.** `neutron outdated` offers three incremental levels: Level 1 (registry scan — deprecation, peer conflicts, risk scores), Level 2 (static API surface diff — tarball download + export diffing), Level 3 (live validation — temp dir + typecheck + tests). Each level is offered after the previous completes. `--deep` forces L2, `--verify` forces L1+L2+L3, `--json`/`--ci` stop at L1.
...
- **neutron owns git hooks.** `neutron install` writes hooks to `.git/hooks/`. No Husky, no commitlint — neutron handles pre-commit, commit-msg, post-merge, and post-checkout.
...
- **Plugins own pipeline steps.** Domain plugins (e.g., neutron-openapi) decompose bridges into discrete steps via `buildPipeline()`. The pipeline runner executes steps sequentially with per-step timing and SHA-256 output hashing for change detection. The `run` field on bridges is a fallback for when no plugin claims the bridge.
...
- **Plugins inform watch paths during init.** Domain plugins suggest watch paths based on framework detection (e.g., neutron-openapi finds Elysia routes). Ecosystem plugins suggest based on package structure. Suggestions are presented for user confirmation, not applied blindly.
...
- **Tool resolution for lint/format.** neutron bundles oxlint and oxfmt as direct dependencies. TS plugin checks workspace `node_modules/.bin/` first (user override), then neutron's own bundled `node_modules/.bin/`, then `eslint`/`prettier` as fallbacks. Dart plugin uses `dart analyze`/`dart format` from PATH. Missing tools produce a warning, not an error.
- **Config is ecosystem-centric, not tool-centric.** `format.typescript` holds oxfmt options, `format.dart` holds dart format options. `lint.typescript` holds oxlint categories/rules, `lint.dart` holds dart analyze options. Users never write tool names — neutron picks the right tool per ecosystem. `format.ignore` and `lint.ignore` apply across all ecosystems.
...
- **Lint/format config lives in neutron.yml.** The `lint` and `format` sections configure tools per ecosystem. The TS plugin generates temporary `.oxlintrc.json` and `.oxfmtrc.json` at runtime in `node_modules/.cache/neutron/` and passes `--config` flags to the tools. No config files committed to the repo. `neutron init` migrates existing configs into the ecosystem-centric structure.
- **JSON schema for VS Code autocomplete.** `scripts/generate-schema.ts` produces `schema.json` at build time. The `yaml-language-server` comment in generated neutron.yml points to `node_modules/@oddlantern/neutron/schema.json`. Schema ships with the npm package.
- **Commits is a top-level config section.** Not nested under lint. `neutron init` auto-populates `commits.scopes` from detected package names.
- **Pre-commit is a single command.** `neutron pre-commit` runs format check → lint → workspace check in sequence, stopping on first failure. The pre-commit hook installed by `neutron install` is just `neutron pre-commit`.
...
- **neutron-design is a domain plugin.** Same tier as neutron-openapi. Owns the `tokens.json` schema (Zod), validates tokens, then delegates code generation to ecosystem plugins. neutron-dart generates Flutter themes (ColorScheme, ThemeExtensions, constants, ThemeData). neutron-typescript generates CSS custom properties and TS constants. One `tokens.json`, all ecosystems served.
...
- **Generated output convention: `<source>/generated/<ecosystem>/`.** Domain plugins (openapi, design) write generated code next to the source package, not into consumers. Consumers depend on the generated package via workspace links. The `generated/` directories are gitignored — they are derived output regenerated by `neutron generate` (fresh clone, CI) or `neutron dev` (watch mode). This is consistent across all domain plugins.
...
- **`neutron generate` runs all bridge pipelines.** Non-watch equivalent of what `neutron dev` does on change. Used after fresh clone or in CI. Resolves all bridges, groups by artifact, executes each pipeline.
- **neutron-assets is a domain plugin.** Same tier as neutron-design and neutron-openapi. Scans directories for asset files (SVG, PNG, etc.), infers categories from filename prefixes (e.g., `achievement_*`, `genre_*`, `ui_*`), detects theme variants (light/dark subdirs), then delegates to ecosystem plugins. neutron-dart generates typed Flutter widget classes (SvgPicture wrappers) + pubspec asset declarations. neutron-typescript generates path exports and inlined SVG strings. Class name prefixes are derived from the workspace name in neutron.yml.
- **`neutron rename` cascades workspace name.** Updates neutron.yml, all package.json names (@scope), all pubspec.yaml names (prefix_), then reminds to run `neutron generate` to propagate into generated code. Platform identifiers (iOS bundle ID, Android application ID, Firebase config) are detected and warned about but NOT renamed by default — they are deployment identities. `--include-platform-ids` overrides this for pre-release projects.
- **Plugin directory structure: ecosystem/ + domain/.** Ecosystem plugins live in `builtin/ecosystem/{name}/plugin.ts`. Domain plugins live in `builtin/domain/{name}/plugin.ts`. This makes the two-tier architecture visible in the filesystem. Naming convention for future extraction: `neutron-{name}-ecosystem` and `neutron-{name}-domain`.
...
Enforced by `neutron commit-msg` (via git hook).
...
3. Add ecosystem to `neutron.yml` schema in `src/config/schema.ts` (if new manifest keys needed)
4. Add test fixture packages with the new manifest format
5. Verify `neutron check` discovers and parses them
...
- Do not auto-format on commit hooks. The pre-commit hook runs `neutron pre-commit` (format check + lint + workspace check). It checks formatting but does not auto-fix.
- Do not add CLI framework deps (commander, yargs, etc). The command router is ~30 lines and that's intentional.
- Do not create separate `.oxlintrc.json`, `.oxfmtrc.json`, or `.oxfmtignore` files. All lint and format config lives in the ecosystem-centric `lint` and `format` sections of `neutron.yml` (e.g., `format.typescript`, `lint.typescript.categories`). The TS plugin generates temporary config files at runtime in `node_modules/.cache/neutron/`.
