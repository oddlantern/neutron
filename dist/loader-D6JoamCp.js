...
import { f as YELLOW, l as RED, u as RESET } from "./output-DN5bGOyX.js";
import { t as NEUTRON_ROOT } from "./version-D_35A4VD.js";
import { n as CACHE_DIR } from "./branding-BIVXTc9K.js";
import { t as STANDARD_ACTIONS } from "./types-BZtO9wbl.js";
import { t as runCommand } from "./process-GHUW_O_C.js";
...
const HEADER$11 = "/* GENERATED — DO NOT EDIT. Changes will be overwritten. */";
...
const HEADER$10 = `/* GENERATED — DO NOT EDIT. Changes will be overwritten. */`;
/** Radius values at or above this threshold are treated as unitless (e.g., pill shapes) */
const FULL_RADIUS_THRESHOLD$2 = 999;
/**
* Convert camelCase to kebab-case.
*/
function camelToKebab$2(str) {
...
function generateCSS(tokens) {
	...
	for (const [, ext] of Object.entries(tokens.extensions)) {
		...
		for (const [fieldName, field] of Object.entries(ext.fields)) {
			const varName = `--${prefix}-${camelToKebab$2(fieldName)}`;
	...
	if (Object.keys(spacing).length > 0) {
		...
		for (const [key, value] of Object.entries(spacing)) lightVars.push(`  --spacing-${camelToKebab$2(key)}: ${value}px;`);
	}
	if (Object.keys(radius).length > 0) {
		lightVars.push("");
		lightVars.push("  /* Radius */");
		for (const [key, value] of Object.entries(radius)) {
			const unit = value >= FULL_RADIUS_THRESHOLD$2 ? "" : "px";
			lightVars.push(`  --radius-${camelToKebab$2(key)}: ${value}${unit};`);
		}
	}
	if (Object.keys(iconSize).length > 0) {
		lightVars.push("");
		lightVars.push("  /* Icon size */");
		for (const [key, value] of Object.entries(iconSize)) lightVars.push(`  --icon-${camelToKebab$2(key)}: ${value}px;`);
	}
	lines.push(":root {");
...
*/
function isValidatedTokens$3(value) {
	if (!isRecord(value)) return false;
...
async function executeDesignTokenGeneration$1(pkg, root, context) {
	...
	if (!isValidatedTokens$3(rawDomainData)) return {
		success: false,
		duration: 0,
...
//#endregion
//#region src/plugins/builtin/ecosystem/typescript/tailwind-codegen.ts
...
const FULL_RADIUS_THRESHOLD$1 = 999;
...
*/
function generateTailwindCSS(tokens) {
	const { color, spacing, radius, iconSize } = tokens.standard;
	const lines = [CSS_HEADER];
	lines.push("@theme {");
	for (const [key, entry] of Object.entries(color)) lines.push(`  --color-${camelToKebab$1(key)}: ${entry.light};`);
	for (const [, ext] of Object.entries(tokens.extensions)) {
		lines.push("");
		for (const [fieldName, field] of Object.entries(ext.fields)) lines.push(`  --color-${ext.meta.getter}-${camelToKebab$1(fieldName)}: ${field.light};`);
	}
	if (Object.keys(spacing).length > 0) {
		lines.push("");
		for (const [key, value] of Object.entries(spacing)) lines.push(`  --spacing-${camelToKebab$1(key)}: ${value}px;`);
	}
	if (Object.keys(radius).length > 0) {
		lines.push("");
		for (const [key, value] of Object.entries(radius)) {
			const cssValue = value >= FULL_RADIUS_THRESHOLD$1 ? "9999px" : `${value}px`;
			lines.push(`  --radius-${camelToKebab$1(key)}: ${cssValue};`);
		}
	}
	if (Object.keys(iconSize).length > 0) {
		lines.push("");
		for (const [key, value] of Object.entries(iconSize)) lines.push(`  --size-icon-${camelToKebab$1(key)}: ${value}px;`);
	}
	lines.push("}");
	lines.push("");
	lines.push("@media (prefers-color-scheme: dark) {");
	lines.push("  @theme {");
	for (const [key, entry] of Object.entries(color)) lines.push(`    --color-${camelToKebab$1(key)}: ${entry.dark};`);
	for (const [, ext] of Object.entries(tokens.extensions)) for (const [fieldName, field] of Object.entries(ext.fields)) lines.push(`    --color-${ext.meta.getter}-${camelToKebab$1(fieldName)}: ${field.dark};`);
	lines.push("  }");
	lines.push("}");
	lines.push("");
	return lines.join("\n");
...
*/
function generateTailwindTS(tokens) {
	const { spacing, radius, iconSize } = tokens.standard;
...
function isValidatedTokens$2(value) {
	if (!isRecord(value)) return false;
	if (!isRecord(value["standard"])) return false;
	return typeof value["standard"]["color"] === "object" && value["standard"]["color"] !== null;
}
async function executeTailwindGeneration(_pkg, _root, context) {
	const start = performance.now();
	const rawDomainData = context.domainData;
	if (!isValidatedTokens$2(rawDomainData)) return {
		success: false,
		duration: 0,
		summary: "No token data provided — design plugin must validate first"
	};
...
//#endregion
//#region src/plugins/builtin/ecosystem/typescript/bootstrap-codegen.ts
const HEADER$9 = "// GENERATED — DO NOT EDIT. Changes will be overwritten.\n";
/** Radius values at or above this threshold are treated as pill/full */
...
*/
function generateBootstrapScss(tokens) {
	const { color, spacing, radius } = tokens.standard;
	const lines = [HEADER$9];
	lines.push("// Colors (light theme)");
	for (const [key, entry] of Object.entries(color)) lines.push(`$${camelToKebab(key)}: ${entry.light};`);
	lines.push("");
	for (const [, ext] of Object.entries(tokens.extensions)) {
		lines.push(`// ${ext.meta.className}`);
		for (const [fieldName, field] of Object.entries(ext.fields)) lines.push(`$${ext.meta.getter}-${camelToKebab(fieldName)}: ${field.light};`);
		lines.push("");
	}
	if (Object.keys(spacing).length > 0) {
		lines.push("// Spacing");
		for (const [key, value] of Object.entries(spacing)) lines.push(`$spacing-${camelToKebab(key)}: ${value}px;`);
		lines.push("");
	}
	if (Object.keys(radius).length > 0) {
	...
	for (const [key, entry] of Object.entries(color)) lines.push(`  $${camelToKebab(key)}: ${entry.dark} !global;`);
	for (const [, ext] of Object.entries(tokens.extensions)) for (const [fieldName, field] of Object.entries(ext.fields)) lines.push(`  $${ext.meta.getter}-${camelToKebab(fieldName)}: ${field.dark} !global;`);
	lines.push("}");
	lines.push("");
	return lines.join("\n");
}
...
async function executeBootstrapGeneration(_pkg, _root, context) {
	const start = performance.now();
	const rawDomainData = context.domainData;
...
//#endregion
//#region src/plugins/builtin/ecosystem/typescript/schema-codegen.ts
const HEADER$8 = "/* GENERATED — DO NOT EDIT. Changes will be overwritten. */\n";
function tsType(prop) {
	if (prop.enumValues) return prop.enumValues.map((v) => `"${v}"`).join(" | ");
...
function generateTypeScript(schema) {
	...
	for (const def of schema.definitions) sections.push(generateInterface(def));
	return sections.join("\n\n") + "\n";
}
async function executeSchemaGeneration$5(_pkg, _root, context) {
	const start = performance.now();
	const schema = context.domainData;
	const outDir = context.outputDir;
...
const WATCH_PATTERNS$5 = ["src/**/*.ts", "src/**/*.tsx"];
...
const typescriptPlugin = {
	...
	async execute(action, pkg, root, context) {
		...
		if (action === ACTION_GENERATE_DESIGN_TOKENS_CSS) switch (context.bridgeFormat ?? "css") {
			case "tailwind": return executeTailwindGeneration(pkg, root, context);
			case "bootstrap": return executeBootstrapGeneration(pkg, root, context);
			default: return executeDesignTokenGeneration$1(pkg, root, context);
		}
		...
		if (action === ACTION_GENERATE_SCHEMA_TS) return executeSchemaGeneration$5(pkg, root, context);
...
const HEADER$7 = `// GENERATED — DO NOT EDIT. Changes will be overwritten.`;
/**
...
function generateThemeExtensions(tokens) {
	...
	if (extEntries.length === 0) return `${HEADER$7}\n\n// No extensions defined in tokens.json\n`;
	const lines = [
		HEADER$7,
		"",
		"import 'dart:ui';",
...
const HEADER$6 = `// GENERATED — DO NOT EDIT. Changes will be overwritten.`;
/**
* Expand 3-char hex (#RGB) → 6-char (#RRGGBB).
* Returns the input unchanged if not a 3-char hex.
*/
...
const HEADER$5 = "// GENERATED — DO NOT EDIT. Changes will be overwritten.";
/**
...
//#endregion
//#region src/plugins/builtin/ecosystem/dart/schema-codegen.ts
const HEADER$4 = "// GENERATED — DO NOT EDIT. Changes will be overwritten.\n";
function dartType(prop) {
	if (prop.enumValues) return `${prop.name[0].toUpperCase()}${prop.name.slice(1)}`;
	if (prop.ref) return prop.ref;
	switch (prop.type) {
		case "string": return "String";
...
function dartItemType(items) {
	switch (items) {
		case "string": return "String";
		case "number": return "double";
		case "integer": return "int";
		case "boolean": return "bool";
		case void 0: return "dynamic";
...
function generateEnum$2(prop) {
	return `enum ${`${prop.name[0].toUpperCase()}${prop.name.slice(1)}`} {\n${(prop.enumValues ?? []).map((v) => `  ${snakeCase$2(v)}`).join(",\n")}\n}`;
}
function generateClass$1(def) {
	const lines = [];
	const enums = [];
	if (def.description) lines.push(`/// ${def.description}`);
	lines.push(`class ${def.name} {`);
	for (const prop of def.properties) {
		if (prop.enumValues) enums.push(generateEnum$2(prop));
		const type = dartType(prop);
		const nullable = prop.nullable || !prop.required ? "?" : "";
		if (prop.description) lines.push(`  /// ${prop.description}`);
		lines.push(`  final ${type}${nullable} ${snakeCase$2(prop.name)};`);
	}
	lines.push("");
	const params = def.properties.map((prop) => {
		return `    ${prop.required && !prop.nullable ? "required " : ""}this.${snakeCase$2(prop.name)},`;
	});
...
function generateDart(schema) {
	const sections = [HEADER$4];
	for (const def of schema.definitions) sections.push(generateClass$1(def));
	return sections.join("\n\n") + "\n";
}
async function executeSchemaGeneration$4(_pkg, _root, context) {
	const start = performance.now();
	const schema = context.domainData;
	const outDir = context.outputDir;
...
const WATCH_PATTERNS$4 = ["lib/**/*.dart", "bin/**/*.dart"];
...
const dartPlugin = {
	...
	async execute(action, pkg, root, context) {
		...
		switch (action) {
			...
			case ACTION_GENERATE_SCHEMA_DART: return executeSchemaGeneration$4(pkg, root, context);
			default: return {
				success: false,
				duration: 0,
...
function pyItemType(items) {
	switch (items) {
		case "string": return "str";
		case "number": return "float";
		case "integer": return "int";
		case "boolean": return "bool";
		case void 0: return "object";
		default: return items;
	}
}
...
function generateEnum$1(prop) {
	return `\nclass ${`${prop.name[0].toUpperCase()}${prop.name.slice(1)}`}(str, Enum):\n${(prop.enumValues ?? []).map((v) => `    ${snakeCase$1(v).toUpperCase()} = "${v}"`).join("\n")}\n`;
}
function generateDataclass(def) {
	const lines = [];
	const enums = [];
	if (def.description) lines.push(`    """${def.description}"""\n`);
	for (const prop of def.properties) {
		if (prop.enumValues) enums.push(generateEnum$1(prop));
		let type = pyType(prop);
		if (prop.nullable || !prop.required) type = `Optional[${type}]`;
		const defaultVal = !prop.required ? " = None" : "";
		lines.push(`    ${snakeCase$1(prop.name)}: ${type}${defaultVal}`);
	}
	...
	if (def.description) classLines.push(`    """${def.description}"""\n`);
	classLines.push(...orderedFields);
	return classLines.join("\n");
}
function generatePython(schema) {
	const sections = [HEADER$3];
	for (const def of schema.definitions) sections.push(generateDataclass(def));
	return sections.join("\n\n") + "\n";
}
async function executeSchemaGeneration$3(_pkg, _root, context) {
	const start = performance.now();
	const schema = context.domainData;
	const outDir = context.outputDir;
...
//#endregion
//#region src/plugins/builtin/ecosystem/python/plugin.ts
const WATCH_PATTERNS$3 = ["**/*.py", "pyproject.toml"];
/** Check if a tool is available on PATH */
async function hasTool$1(name, cwd) {
	return (await runCommand("which", [name], cwd)).success;
}
const pythonPlugin = {
	type: "ecosystem",
	name: "python",
	...
	async getWatchPatterns() {
		return WATCH_PATTERNS$3;
	},
	async getActions(_pkg, root) {
		const actions = [];
		const cwd = root;
		if (await hasTool$1("ruff", cwd)) {
		...
		if (await hasTool$1("pytest", cwd)) actions.push(STANDARD_ACTIONS.TEST);
		if (await hasTool$1("mypy", cwd)) actions.push(STANDARD_ACTIONS.TYPECHECK);
		else if (await hasTool$1("pyright", cwd)) actions.push(STANDARD_ACTIONS.TYPECHECK);
		return actions;
	},
	async execute(action, pkg, root, _context) {
		...
		switch (action) {
			case STANDARD_ACTIONS.LINT: {
				const args = ["check"];
				if (lintPy?.select) args.push("--select", lintPy.select.join(","));
				if (lintPy?.ignore) args.push("--ignore", lintPy.ignore.join(","));
				if (lintPy?.targetVersion) args.push("--target-version", lintPy.targetVersion);
				args.push(".");
				return runCommand("ruff", args, cwd);
			}
			case STANDARD_ACTIONS.LINT_FIX: {
				const args = ["check", "--fix"];
				if (lintPy?.fixable) args.push("--fixable", lintPy.fixable.join(","));
				args.push(".");
				return runCommand("ruff", args, cwd);
			}
			case STANDARD_ACTIONS.FORMAT: {
				const args = ["format"];
				if (fmtPy?.lineLength) args.push("--line-length", String(fmtPy.lineLength));
				if (fmtPy?.quoteStyle) args.push("--config", `quote-style="${fmtPy.quoteStyle}"`);
				args.push(".");
				return runCommand("ruff", args, cwd);
			}
			case STANDARD_ACTIONS.FORMAT_CHECK: {
			...
			case STANDARD_ACTIONS.TEST: return runCommand("pytest", [], cwd);
			case STANDARD_ACTIONS.TYPECHECK:
				if (await hasTool$1("mypy", root)) return runCommand("mypy", ["."], cwd);
				return runCommand("pyright", ["."], cwd);
			case "generate-schema-python": return executeSchemaGeneration$3(pkg, root, _context);
			default: return {
				success: false,
				duration: 0,
				summary: `Unknown action: ${action}`
	...
	async canHandleDomainArtifact(domain, _artifact, _pkg, _root) {
		if (domain === "openapi") return {
...
//#endregion
//#region src/plugins/builtin/ecosystem/rust/schema-codegen.ts
const HEADER$2 = "// GENERATED — DO NOT EDIT. Changes will be overwritten.\n\nuse serde::{Deserialize, Serialize};\n";
function rustType(prop) {
	if (prop.enumValues) return `${prop.name[0].toUpperCase()}${prop.name.slice(1)}`;
	if (prop.ref) return prop.ref;
	switch (prop.type) {
		case "string": return "String";
...
function rustItemType(items) {
	switch (items) {
		case "string": return "String";
		case "number": return "f64";
		case "integer": return "i64";
		case "boolean": return "bool";
		case void 0: return "serde_json::Value";
		default: return items;
	}
}
...
function generateEnum(prop) {
	return `\n#[derive(Debug, Clone, Serialize, Deserialize)]\npub enum ${`${prop.name[0].toUpperCase()}${prop.name.slice(1)}`} {\n${(prop.enumValues ?? []).map((v) => `    ${v[0].toUpperCase()}${v.slice(1)}`).join(",\n")},\n}`;
}
function generateStruct$1(def) {
	const lines = [];
	const enums = [];
	if (def.description) lines.push(`/// ${def.description}`);
	...
	for (const prop of def.properties) {
		if (prop.enumValues) enums.push(generateEnum(prop));
		let type = rustType(prop);
		if (prop.nullable || !prop.required) type = `Option<${type}>`;
		if (prop.description) lines.push(`    /// ${prop.description}`);
...
function generateRust(schema) {
	const sections = [HEADER$2];
	for (const def of schema.definitions) sections.push(generateStruct$1(def));
	return sections.join("\n\n") + "\n";
}
async function executeSchemaGeneration$2(_pkg, _root, context) {
	const start = performance.now();
	const schema = context.domainData;
	const outDir = context.outputDir;
...
//#endregion
//#region src/plugins/builtin/ecosystem/rust/plugin.ts
const WATCH_PATTERNS$2 = ["src/**/*.rs", "Cargo.toml"];
const rustPlugin = {
	type: "ecosystem",
	name: "rust",
	manifest: "Cargo.toml",
	...
	async execute(action, pkg, root, _context) {
		...
		switch (action) {
			case STANDARD_ACTIONS.LINT: {
				const args = ["clippy"];
				if (lintRs?.features) args.push("--features", lintRs.features.join(","));
				args.push("--");
				if (lintRs?.denyWarnings !== false) args.push("-D", "warnings");
				return runCommand("cargo", args, cwd);
			}
			case STANDARD_ACTIONS.LINT_FIX: return runCommand("cargo", [
				"clippy",
				"--fix",
				"--allow-dirty",
				"--allow-staged"
			], cwd);
			case STANDARD_ACTIONS.FORMAT: return runCommand("cargo", ["fmt"], cwd);
			case STANDARD_ACTIONS.FORMAT_CHECK: return runCommand("cargo", ["fmt", "--check"], cwd);
			case STANDARD_ACTIONS.TEST: return runCommand("cargo", ["test"], cwd);
			case STANDARD_ACTIONS.BUILD: return runCommand("cargo", ["build"], cwd);
			case "generate-schema-rust": return executeSchemaGeneration$2(pkg, root, _context);
			default: return {
				success: false,
				duration: 0,
				summary: `Unknown action: ${action}`
...
//#endregion
//#region src/plugins/builtin/ecosystem/go/schema-codegen.ts
...
function goItemType(items) {
	switch (items) {
		case "string": return "string";
		case "number": return "float64";
		case "integer": return "int64";
		case "boolean": return "bool";
		case void 0: return "interface{}";
		default: return items;
	}
}
...
function generateStruct(def) {
	const lines = [];
	if (def.description) lines.push(`// ${def.name} — ${def.description}`);
...
function generateGo(schema) {
	const sections = [HEADER$1];
	for (const def of schema.definitions) sections.push(generateStruct(def));
	return sections.join("\n\n") + "\n";
}
async function executeSchemaGeneration$1(_pkg, _root, context) {
	const start = performance.now();
	const schema = context.domainData;
	const outDir = context.outputDir;
...
//#endregion
//#region src/plugins/builtin/ecosystem/go/plugin.ts
const WATCH_PATTERNS$1 = [
	"**/*.go",
	"go.mod",
	"go.sum"
];
/** Check if a tool is available on PATH */
async function hasTool(name, cwd) {
	return (await runCommand("which", [name], cwd)).success;
}
const goPlugin = {
	type: "ecosystem",
	...
	async getActions(_pkg, root) {
		...
		if (await hasTool("golangci-lint", root)) actions.push(STANDARD_ACTIONS.LINT);
		return actions;
	},
	async execute(action, pkg, root, _context) {
		const cwd = `${root}/${pkg.path}`;
		switch (action) {
			case STANDARD_ACTIONS.LINT: {
				...
				const args = ["run"];
				if (lintGo?.enable) args.push("--enable", lintGo.enable.join(","));
				if (lintGo?.disable) args.push("--disable", lintGo.disable.join(","));
				if (lintGo?.timeout) args.push("--timeout", lintGo.timeout);
				return runCommand("golangci-lint", args, cwd);
			}
			case STANDARD_ACTIONS.FORMAT: return runCommand("gofmt", ["-w", "."], cwd);
			case STANDARD_ACTIONS.FORMAT_CHECK: {
			...
			case STANDARD_ACTIONS.TEST: return runCommand("go", ["test", "./..."], cwd);
			case STANDARD_ACTIONS.BUILD: return runCommand("go", ["build", "./..."], cwd);
			case STANDARD_ACTIONS.TYPECHECK: return runCommand("go", ["vet", "./..."], cwd);
			case "generate-schema-go": return executeSchemaGeneration$1(pkg, root, _context);
			default: return {
				success: false,
				duration: 0,
				summary: `Unknown action: ${action}`
...
//#endregion
//#region src/plugins/builtin/ecosystem/php/schema-codegen.ts
const HEADER = "<?php\n\n// GENERATED — DO NOT EDIT. Changes will be overwritten.\n\ndeclare(strict_types=1);\n";
function phpType(prop) {
	if (prop.ref) return prop.ref;
	switch (prop.type) {
		case "string": return "string";
		case "number":
...
function generateClass(def) {
	...
	for (const prop of def.properties) {
		...
		if (prop.description) params.push(`        /** ${prop.description} */`);
...
function generatePhp(schema) {
	const sections = [HEADER];
	for (const def of schema.definitions) sections.push(generateClass(def));
	return sections.join("\n\n") + "\n";
}
async function executeSchemaGeneration(_pkg, _root, context) {
	const start = performance.now();
	const schema = context.domainData;
	const outDir = context.outputDir;
...
//#endregion
//#region src/plugins/builtin/ecosystem/php/plugin.ts
...
function resolveVendorBin(name, pkgDir) {
...
const phpPlugin = {
	...
	async getActions(pkg, root) {
		const actions = [];
		const pkgDir = join(root, pkg.path);
		if (existsSync(join(pkgDir, "vendor", "bin", "phpstan")) || existsSync(join(pkgDir, "vendor", "bin", "psalm"))) actions.push(STANDARD_ACTIONS.LINT);
		if (existsSync(join(pkgDir, "vendor", "bin", "php-cs-fixer")) || existsSync(join(pkgDir, "vendor", "bin", "pint"))) actions.push(STANDARD_ACTIONS.FORMAT, STANDARD_ACTIONS.FORMAT_CHECK);
		if (existsSync(join(pkgDir, "vendor", "bin", "phpunit")) || existsSync(join(pkgDir, "vendor", "bin", "pest"))) actions.push(STANDARD_ACTIONS.TEST);
		return actions;
	},
	async execute(action, pkg, root, _context) {
		const cwd = join(root, pkg.path);
		switch (action) {
			case STANDARD_ACTIONS.LINT: {
				...
				const bin = existsSync(join(cwd, "vendor", "bin", "phpstan")) ? resolveVendorBin("phpstan", cwd) : resolveVendorBin("psalm", cwd);
				const args = ["analyse"];
				if (lintPhp?.level !== void 0 && bin.includes("phpstan")) args.push("--level", String(lintPhp.level));
				if (lintPhp?.paths) args.push(...lintPhp.paths);
				return runCommand(bin, args, cwd);
			}
			case STANDARD_ACTIONS.FORMAT: {
				const bin = existsSync(join(cwd, "vendor", "bin", "php-cs-fixer")) ? resolveVendorBin("php-cs-fixer", cwd) : resolveVendorBin("pint", cwd);
				return runCommand(bin, bin.includes("php-cs-fixer") ? ["fix"] : [], cwd);
			}
			case STANDARD_ACTIONS.FORMAT_CHECK: {
				const bin = existsSync(join(cwd, "vendor", "bin", "php-cs-fixer")) ? resolveVendorBin("php-cs-fixer", cwd) : resolveVendorBin("pint", cwd);
				return runCommand(bin, bin.includes("php-cs-fixer") ? [
					"fix",
					"--dry-run",
					"--diff"
				] : ["--test"], cwd);
			}
			case STANDARD_ACTIONS.TEST: return runCommand(existsSync(join(cwd, "vendor", "bin", "pest")) ? resolveVendorBin("pest", cwd) : resolveVendorBin("phpunit", cwd), [], cwd);
			case "generate-schema-php": return executeSchemaGeneration(pkg, root, _context);
			default: return {
				success: false,
				duration: 0,
				summary: `Unknown action: ${action}`
...
const assetsPlugin = {
	...
	async generateDownstream(artifact, targets, root, context) {
		...
		const handlers = await context.findEcosystemHandlers(DOMAIN_NAME$2, artifact);
...
//#endregion
//#region src/plugins/builtin/domain/design/formats.ts
/**
...
*/
function detectDesignFormat(pkg) {
	const depNames = new Set(pkg.dependencies.map((d) => d.name));
	if (depNames.has("tailwindcss")) return "tailwind";
	if (depNames.has("bootstrap") || depNames.has("react-bootstrap") || depNames.has("@ng-bootstrap/ng-bootstrap")) return "bootstrap";
	if (pkg.ecosystem === "dart") return "material3";
	return "css";
}
//#endregion
//#region src/plugins/builtin/shared/wire-dep.ts
/**
...
* @param root - Workspace root absolute path
*/
async function wireGeneratedDependency(consumerPath, generatedPkgDir, ecosystem, root) {
	switch (ecosystem) {
		case "typescript": return wireTypeScript(consumerPath, generatedPkgDir, root);
		case "dart": return wireDart(consumerPath, generatedPkgDir, root);
		default: return false;
	}
}
async function wireTypeScript(consumerPath, generatedPkgDir, root) {
	const consumerManifest = join(root, consumerPath, "package.json");
	const generatedManifest = join(generatedPkgDir, "package.json");
	if (!existsSync(consumerManifest) || !existsSync(generatedManifest)) return false;
	const genRaw = await readFile(generatedManifest, "utf-8");
	const genPkg = JSON.parse(genRaw);
	if (!isRecord(genPkg) || typeof genPkg["name"] !== "string") return false;
	const genName = genPkg["name"];
	const consumerRaw = await readFile(consumerManifest, "utf-8");
	const consumer = JSON.parse(consumerRaw);
	if (!isRecord(consumer)) return false;
	const deps = isRecord(consumer["dependencies"]) ? consumer["dependencies"] : {};
	if (genName in deps) return false;
	deps[genName] = "workspace:*";
	consumer["dependencies"] = deps;
	const indent = consumerRaw.match(/^(\s+)"/m)?.[1] ?? "  ";
	await writeFile(consumerManifest, JSON.stringify(consumer, null, indent) + "\n", "utf-8");
	return true;
}
async function wireDart(consumerPath, generatedPkgDir, root) {
	const consumerManifest = join(root, consumerPath, "pubspec.yaml");
	const generatedManifest = join(generatedPkgDir, "pubspec.yaml");
	if (!existsSync(consumerManifest) || !existsSync(generatedManifest)) return false;
	const genName = parseDocument(await readFile(generatedManifest, "utf-8")).get("name");
	if (typeof genName !== "string") return false;
	const consumerDoc = parseDocument(await readFile(consumerManifest, "utf-8"));
	const deps = consumerDoc.get("dependencies", true);
	if (isMap(deps) && deps.has(genName)) return false;
	const relPath = relative(join(root, consumerPath), generatedPkgDir);
	if (!isMap(deps)) consumerDoc.set("dependencies", { [genName]: { path: relPath } });
	else deps.set(genName, { path: relPath });
	await writeFile(consumerManifest, consumerDoc.toString(), "utf-8");
	return true;
}
//#endregion
//#region src/plugins/builtin/domain/design/plugin.ts
const DOMAIN_NAME$1 = "design-tokens";
/**
...
const designPlugin = {
	...
	async buildPipeline(_source, artifact, targets, root, context) {
		...
		const targetPaths = new Set(targets.map((t) => t.path));
		const relevantHandlers = handlers.filter((h) => targetPaths.has(h.pkg.path));
		const bridge = context.graph.bridges.find((b) => b.artifact === artifact && b.source === _source.path);
		const consumerFormats = /* @__PURE__ */ new Map();
		if (bridge) for (const consumer of bridge.consumers) {
		...
		for (const handler of relevantHandlers) {
			...
			const resolvedFormat = consumerFormats.get(handler.pkg.path) ?? detectDesignFormat(handler.pkg);
			steps.push({
				name: `generate-${handler.plugin.name}`,
				plugin: handler.plugin.name,
				...
				execute: async () => {
					...
					if (result.success && !context.dryRun) {
						for (const target of targets) if (target.ecosystem === handler.plugin.name) await wireGeneratedDependency(target.path, outputDir, target.ecosystem, root);
					}
					return result;
				}
			});
...
async function detectFrameworkAdapter(pkgPath, root) {
	const { detectAdapter } = await import("./adapters-CwESO6z_.js");
	try {
...
//#endregion
//#region src/plugins/builtin/domain/schema/plugin.ts
...
*/
function parseProperties(properties, requiredFields) {
	const result = [];
	for (const [name, rawProp] of Object.entries(properties)) {
		...
		if (variants) {
			...
			if (variants.some((v) => isRecord(v) && v["type"] === "null") && nonNull.length === 1 && isRecord(nonNull[0])) {
				nullable = true;
				actualProp = nonNull[0];
			}
		}
		const type = TYPE_MAP[typeof actualProp["type"] === "string" ? actualProp["type"] : "string"] ?? "string";
		const enumValues = Array.isArray(actualProp["enum"]) ? actualProp["enum"].filter((v) => typeof v === "string") : void 0;
		const format = typeof actualProp["format"] === "string" ? actualProp["format"] : void 0;
		const description = typeof actualProp["description"] === "string" ? actualProp["description"] : void 0;
		let items;
		if (type === "array" && isRecord(actualProp["items"])) {
			const itemsObj = actualProp["items"];
			if (typeof itemsObj["$ref"] === "string") items = refToName(itemsObj["$ref"]);
			else if (typeof itemsObj["type"] === "string") items = itemsObj["type"];
		}
		let ref;
		if (typeof actualProp["$ref"] === "string") ref = refToName(actualProp["$ref"]);
		result.push({
			name,
...
function validateSchema(raw) {
	...
	const errors = [];
	const definitions = [];
	const defs = isRecord(raw["$defs"]) ? raw["$defs"] : isRecord(raw["definitions"]) ? raw["definitions"] : null;
	if (defs) for (const [name, defRaw] of Object.entries(defs)) {
		if (!isRecord(defRaw)) {
			errors.push({
				path: `$defs.${name}`,
	...
	if (isRecord(raw["properties"])) {
		const def = parseDefinition(typeof raw["title"] === "string" ? raw["title"] : typeof raw["$id"] === "string" ? idToName(raw["$id"]) : "Root", raw, errors);
		if (def) definitions.push(def);
	}
	if (definitions.length === 0 && errors.length === 0) errors.push({
		path: "$",
		message: "No type definitions found. Schema must have properties or $defs."
	});
	return {
		success: errors.length === 0,
		data: errors.length === 0 ? { definitions } : void 0,
		errors
	};
}
function parseDefinition(name, raw, errors) {
	const props = raw["properties"];
	if (!isRecord(props)) {
		errors.push({
			path: name,
			message: "Definition has no properties"
		});
		return null;
	}
	const requiredRaw = raw["required"];
	const requiredFields = new Set(Array.isArray(requiredRaw) ? requiredRaw.filter((r) => typeof r === "string") : []);
	return {
		name,
		description: typeof raw["description"] === "string" ? raw["description"] : void 0,
		properties: parseProperties(props, requiredFields)
	};
}
/** Extract a type name from a $id URL like "https://example.com/user.schema.json" */
function idToName(id) {
	return (id.split("/").pop() ?? id).replace(/\.schema\.json$/, "").replace(/\.json$/, "");
}
const schemaPlugin = {
	type: "domain",
	name: "schema",
	async detectBridge(artifact, root) {
		if (!artifact.endsWith(".schema.json")) return false;
		try {
			const content = await readFile(join(root, artifact), "utf-8");
	...
	async exportArtifact(_source, artifact, root) {
		...
		try {
			const content = await readFile(join(root, artifact), "utf-8");
			const result = validateSchema(JSON.parse(content));
			if (!result.success) {
	...
	async generateDownstream(artifact, targets, root, context) {
		...
		for (const handler of relevantHandlers) {
			...
			const result = await handler.plugin.execute(handler.capability.action, handler.pkg, root, ctxWithSchema);
			results.push(result);
		}
		return results;
	},
		...
		for (const handler of relevantHandlers) {
			...
			steps.push({
				...
				execute: async () => {
					...
					return handler.plugin.execute(handler.capability.action, handler.pkg, root, ctxWithSchema);
				}
			});
		}
		return steps;
...
//#endregion
export { loadPlugins as t };

//# sourceMappingURL=loader-D6JoamCp.js.map