#!/usr/bin/env node
import { c as YELLOW, o as RED, s as RESET } from "./output-D1Xg1ws_.js";
...
*/
function detectExtensionFieldType(value) {
	if (typeof value === "number") return "number";
	if (typeof value === "string") return /^#[0-9a-fA-F]{6}$/.test(value) ? "color-static" : "string";
	if (typeof value === "object" && value !== null) {
		const obj = value;
		if (typeof obj["light"] === "string" && typeof obj["dark"] === "string") return "color";
		if (typeof obj["light"] === "number" && typeof obj["dark"] === "number") return "number-themed";
	}
	return "string";
}
/**
	...
	for (const [name, ext] of Object.entries(extensions)) {
		...
		for (const [fieldName, fieldValue] of Object.entries(ext)) {
			...
			fields.set(fieldName, {
				name: fieldName,
				type: detectExtensionFieldType(fieldValue),
				value: fieldValue
			});
		}
		resolved.push({
...
*/
function lerpExpr(fieldName, type) {
	switch (type) {
		case "color":
...
function generateThemeExtensions(tokens) {
	...
	for (const ext of extensions) for (const [, field] of ext.fields) if (field.type === "number" || field.type === "number-themed") needsLerpDouble = true;
	if (needsLerpDouble) {}
	let extIndex = 0;
	for (const ext of extensions) {
		...
		lines.push("  });");
		lines.push("");
		for (const field of fields) lines.push(`  final ${dartType(field.type)} ${field.name};`);
		lines.push("");
		...
		for (const field of fields) lines.push(`    ${dartType(field.type)}? ${field.name},`);
		lines.push("  }) {");
		...
		lines.push(`    return ${ext.name}(`);
		for (const field of fields) lines.push(`      ${field.name}: ${lerpExpr(field.name, field.type)},`);
		lines.push("    );");
		lines.push("  }");
		lines.push("}");
		extIndex++;
...
//# sourceMappingURL=registry-BG_Yc1It.js.map