...
function detectOxlintPlugins(pkg, root) {
	...
	try {
		...
		for (const [dep, depPlugins] of DEP_PLUGIN_MAP) if (hasDep$1(manifest, dep)) for (const p of depPlugins) plugins.add(p);
	} catch {}
	return [...plugins];
}
...
const dartPlugin = {
	...
	async getActions(pkg, root) {
		try {
			...
			if (hasDep(manifest, "swagger_parser")) actions.push(ACTION_GENERATE_API);
			return actions;
		} catch {
...
//#endregion
export { STANDARD_ACTIONS as i, loadPlugins as n, isRecord as r, PluginRegistry as t };

//# sourceMappingURL=registry-ibd6msB5.js.map