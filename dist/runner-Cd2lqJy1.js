import { a as GREEN, f as YELLOW, l as RED, n as CYAN, o as MAGENTA, r as DIM, t as BOLD, u as RESET } from "./output-DN5bGOyX.js";
...
async function resolveBridges(bridges, packages, registry, root) {
	...
	for (const bridge of bridges) {
		...
		for (const consumer of bridge.consumers) {
			...
			if (!consumerPkg) {
				console.error(`${YELLOW}warn:${RESET} bridge consumer "${consumer.path}" not found in graph`);
				continue;
			}
...
function printBridgeSummary(resolved, registry) {
	for (const r of resolved) {
		...
		if (r.domain) {
			const plugins = [`neutron-${r.domain.name}`];
			if (r.sourcePlugin) plugins.push(`neutron-${r.sourcePlugin.name}`);
			for (const t of r.targets) {
				const eco = registry.getEcosystemForPackage(t);
				if (eco && !plugins.includes(`neutron-${eco.name}`)) plugins.push(`neutron-${eco.name}`);
			}
...
function groupBridgesByArtifact(bridges) {
	...
	for (const bridge of bridges) {
		if (!bridge.domain?.buildPipeline) {
			groups.set(`__single__${bridge.bridge.source}__${bridge.bridge.consumers.map((c) => c.path).join(",")}`, [bridge]);
			continue;
		}
...
//# sourceMappingURL=runner-Cd2lqJy1.js.map