...
async function main() {
	...
	if (command === "check") {
		...
		const { runCheck } = await import("./check-DMsxC53B.js");
	...
	if (command === "init") {
		const { runInit } = await import("./init-Ddgpbb3l.js");
	...
	if (command === "lint") {
		...
		const { runLint } = await import("./lint-BcOmeY6K.js");
	...
	if (command === "fmt") {
		...
		const pkg = getFlagValue(args, "--package");
		const ecosystem = getFlagValue(args, "--ecosystem");
		const { runFmt } = await import("./fmt-DY_gWwDy.js");
	...
	if (command === "build") {
		const quiet = args.includes("--quiet");
		const pkg = getFlagValue(args, "--package");
		const { runBuild } = await import("./build-CFNW784j.js");
	...
	if (command === "pre-commit") {
		const { runPreCommit } = await import("./pre-commit-DXuS6PCd.js");
	...
	if (command === "commit-msg") {
		...
		const { runCommitMsg } = await import("./commit-msg-DcRTPZ_y.js");
