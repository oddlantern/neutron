...
async function main() {
	...
	if (command === "check") {
		...
		const { runCheck } = await import("./check-yCF5--1I.js");
	...
	if (command === "init") {
		const { runInit } = await import("./init-R27XmzE0.js");
	...
	if (command === "lint") {
		...
		const { runLint } = await import("./lint-BihbvCq4.js");
	...
	if (command === "fmt") {
		...
		const pkg = getFlagValue(args, "--package");
		const ecosystem = getFlagValue(args, "--ecosystem");
		const { runFmt } = await import("./fmt-CX7aluAQ.js");
	...
	if (command === "build") {
		const quiet = args.includes("--quiet");
		const pkg = getFlagValue(args, "--package");
		const { runBuild } = await import("./build-CTkyaSbj.js");
	...
	if (command === "pre-commit") {
		const { runPreCommit } = await import("./pre-commit-LYrQ4zDy.js");
	...
	if (command === "commit-msg") {
		...
		const { runCommitMsg } = await import("./commit-msg-Cjw_UCak.js");
