...
async function main() {
	...
	if (command === "init") {
		const { runInit } = await import("./init-Cyax6lCO.js");
	...
	if (command === "dev") {
		...
		const { runDev } = await import("./dev-CcE2PEUo.js");
	...
	if (command === "lint") {
		...
		const { runLint } = await import("./lint-BfrmEl8c.js");
	...
	if (command === "fmt") {
		...
		const pkg = getFlagValue(args, "--package");
		const ecosystem = getFlagValue(args, "--ecosystem");
		const { runFmt } = await import("./fmt-CKcpJeB_.js");
	...
	if (command === "generate") {
		...
		const dryRun = args.includes("--dry-run");
		const { runGenerate } = await import("./generate-BCcQtTvn.js");
	...
	if (command === "test") {
		...
		const pkg = getFlagValue(args, "--package");
		const ecosystem = getFlagValue(args, "--ecosystem");
		const { runTest } = await import("./test-D197P0P1.js");
	...
	if (command === "build") {
		...
		const all = args.includes("--all");
		const pkg = getFlagValue(args, "--package");
		const { runBuild } = await import("./build-C5a1Quay.js");
	...
	if (command === "ci") {
		const verbose = args.includes("--verbose");
		const { runCi } = await import("./ci-B1qD6sGy.js");
		const exitCode = await runCi(parsers, { verbose });
		process.exit(exitCode);
	}
	if (command === "pre-commit") {
		const { runPreCommit } = await import("./pre-commit-D8fXNL5x.js");
