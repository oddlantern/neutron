...
import { a as GREEN, r as DIM, s as ORANGE, t as BOLD, u as RESET } from "./output-DN5bGOyX.js";
import { t as printBanner } from "./bin.js";
import { c as REPO_URL, i as DISPLAY_NAME, s as PACKAGE_NAME, t as BINARY_NAME } from "./branding-BIVXTc9K.js";
...
async function runPostInitCheck(parsers) {
	...
	const { buildWorkspaceGraph } = await import("./workspace-MAuyT2B8.js").then((n) => n.n);
	const { findVersionMismatches } = await import("./versions-DgYM6vEv.js").then((n) => n.i);
	const { loadLock } = await import("./lock-CWRUuEwv.js").then((n) => n.i);
	const mismatches = findVersionMismatches(await buildWorkspaceGraph(config, root, parsers), await loadLock(root));
	if (mismatches.length === 0) {
		log.warn(`${DIM}Some checks failed. Run${RESET} ${BOLD}neutron check${RESET} ${DIM}to see details.${RESET}`);
		return false;
	}
...
async function runReconciliation(root, configPath, parsers) {
	...
	const existingBridges = (existing.bridges ?? []).map((b) => ({
		...b,
		consumers: (b.consumers ?? (b.target ? [b.target] : [])).map((c) => typeof c === "string" ? c : c.path)
	}));
	...
	if (installHooks) {
		...
		const installResult = await runInstall(root, void 0, existing);
...
async function wirePrepareScript(root) {
	...
	if (current.includes("neutron generate")) return;
	scripts["prepare"] = current ? `${current} && neutron generate` : "neutron generate";
	pkg["scripts"] = scripts;
...
//# sourceMappingURL=init-BOQiLyEX.js.map