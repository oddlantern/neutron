...
const pythonPlugin = {
	...
	async execute(action, pkg, root, _context) {
		...
		switch (action) {
			case STANDARD_ACTIONS.LINT: return runCommand("ruff", ["check", "."], cwd);
			case STANDARD_ACTIONS.LINT_FIX: return runCommand("ruff", [
				"check",
				"--fix",
				"."
			], cwd);
			case STANDARD_ACTIONS.FORMAT: return runCommand("ruff", ["format", "."], cwd);
			case STANDARD_ACTIONS.FORMAT_CHECK: return runCommand("ruff", [
				"format",
				"--check",
				"."
			], cwd);
...
const rustPlugin = {
	...
	async execute(action, pkg, root, _context) {
		const cwd = `${root}/${pkg.path}`;
		switch (action) {
			case STANDARD_ACTIONS.LINT: return runCommand("cargo", [
				"clippy",
				"--",
				"-D",
				"warnings"
...
const goPlugin = {
	...
	async execute(action, pkg, root, _context) {
		const cwd = `${root}/${pkg.path}`;
		switch (action) {
			case STANDARD_ACTIONS.LINT: return runCommand("golangci-lint", ["run"], cwd);
...
const phpPlugin = {
	...
	async execute(action, pkg, root, _context) {
		const cwd = join(root, pkg.path);
		switch (action) {
			case STANDARD_ACTIONS.LINT: return runCommand(existsSync(join(cwd, "vendor", "bin", "phpstan")) ? resolveVendorBin("phpstan", cwd) : resolveVendorBin("psalm", cwd), ["analyse"], cwd);
			case STANDARD_ACTIONS.FORMAT: {
...
//# sourceMappingURL=registry-CkoeCZGV.js.map