...
import { a as formatLevel2Results, c as buildWorkspaceDepsMap, i as formatLevel1Results, l as collectDeps, o as formatLevel3Results, r as formatJsonOutput, t as runLevel1, u as hasFlutterDeps } from "./level1-BXef9QQO.js";
...
async function findUsedSymbols(sourceDir, depName, ecosystem, sourceFiles) {
	...
	const importRegex = ecosystem === "dart" ? buildDartImportRegex(depName) : buildTsImportRegex(depName);
...
const ALLOWED_ORIGINS = ["https://registry.npmjs.org", "https://pub.dev"];
...
async function runLevel2(outdated, root, packages) {
	...
	for (let i = 0; i < outdated.length; i += CONCURRENCY) {
		...
		const batchResults = await Promise.all(batch.map((dep) => {
			if (dep.ecosystem === "dart") return analyzeDartDep(dep, root, dartSourceFiles);
...
async function runLevel3(outdated, root, packages) {
	...
	const results = [];
	const [tsResults, dartResults] = await Promise.all([tsDeps.length > 0 ? validateTypescriptDeps(tsDeps, root, packages) : Promise.resolve([]), dartDeps.length > 0 ? validateDartDeps(dartDeps, root, packages) : Promise.resolve([])]);
...
//# sourceMappingURL=outdated-C51ipVn4.js.map