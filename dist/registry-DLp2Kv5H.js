...
function colorToDart(value) {
	if (value.startsWith("#")) return `Color(0xFF${value.replace("#", "").toUpperCase()})`;
...
function generateThemeExtensions(tokens) {
	...
	if (extEntries.length === 0) return `${HEADER}\n\n// No extensions defined in tokens.json\n`;
	const lines = [
		HEADER,
		"",
		"import 'dart:ui';",
...
const colorValue = z.string().refine((v) => /^#[0-9a-fA-F]{6}$/.test(v) || /^rgba?\(/.test(v), { message: "expected hex (#RRGGBB) or rgba() color" });
...
function validateTokens(raw) {
	...
	for (const key of Object.keys(obj)) {
		...
		if (typeof sectionValue !== "object" || !sectionValue || Array.isArray(sectionValue)) continue;
...
//# sourceMappingURL=registry-DLp2Kv5H.js.map