...
*/
function updateMidoYml(root, oldName, newName) {
...
*/
function updatePackageJson(filePath, oldName, newName) {
	...
	writeFileSync(filePath, content.replace(`"name": "${name}"`, `"name": "${newPkgName}"`), "utf-8");
	return true;
}
/**
...
*/
function updatePubspec(filePath, oldName, newName) {
	if (!existsSync(filePath)) return false;
...
*/
function updatePubspecDependencies(filePath, oldName, newName) {
	if (!existsSync(filePath)) return false;
	const content = readFileSync(filePath, "utf-8");
	const oldPrefix = oldName.replace(/-/g, "_");
	const newPrefix = newName.replace(/-/g, "_");
...
*/
function renamePlatformIdentifiers(root, ecosystemPaths, oldName, newName) {
	const updated = [];
	const appDirs = [...ecosystemPaths, "."];
	for (const appDir of appDirs) for (const entry of PLATFORM_ID_FILES) {
		const filePath = join(root, appDir, entry.path);
	...
	for (const [ecosystem, eco] of Object.entries(config.config.ecosystems)) {
		...
		for (const pkgPath of eco.packages) {
			...
			if (ecosystem === "typescript") {
				...
				if (updatePubspecDependencies(manifestPath, oldName, newName)) console.log(`  ${GREEN}✓${RESET} ${pkgPath}/${manifestName} (dependencies)`);
			}
		}
	}
	if (updatePackageJson(join(root, "package.json"), oldName, newName)) {
...
//# sourceMappingURL=rename-Cdj2OHfa.js.map