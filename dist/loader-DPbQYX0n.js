...
import { f as YELLOW, r as DIM, t as BOLD, u as RESET } from "./output-DN5bGOyX.js";
import { n as VERSION } from "./version-D_35A4VD.js";
import { r as CONFIG_FILENAMES } from "./branding-BIVXTc9K.js";
import { i as configSchema } from "./schema-BipwgjSu.js";
...
function runMigrations(doc) {
	...
	for (const migration of MIGRATIONS) {
		if (semverGte(VERSION, migration.removedAt)) {
			...
			if (migration.run(probe)) throw new Error(`neutron.yml uses a config format that was removed in v${migration.removedAt}: ${migration.label}\n\nAuto-migration is no longer available. Please update your neutron.yml manually.\nIf you're upgrading from a much older version, first install neutron@${previousMinor(migration.removedAt)} which still auto-migrates, then upgrade to the latest.`);
			continue;
		}
...
async function migrateConfig(configPath, raw) {
	...
	if (applied.length > 0) {
		...
		for (const label of applied) console.log(`migrated neutron.yml: ${label}`);
		return {
			applied,
			content: newContent
		};
...
async function loadConfig(startDir) {
	...
	if (!configPath) throw new Error(`No neutron.yml found. Searched upward from ${searchFrom}\nCreate a neutron.yml in your workspace root, or run "neutron init" to generate one.`);
...
//# sourceMappingURL=loader-DPbQYX0n.js.map