import { f as YELLOW, l as RED, r as DIM, u as RESET } from "./output-DN5bGOyX.js";
...
import { a as printStartup, c as logChange, d as logStep, f as logWaiting, i as printBridgeSummary, l as logDebug, n as groupBridgesByArtifact, o as resolveBridges, r as matchesBridge, t as executeBridgeGroup, u as logFail } from "./runner-Cd2lqJy1.js";
...
async function runDev(parsers, options = {}) {
	...
	async function startSession() {
		...
		for (const r of resolved) {
			const debouncer = createDebouncer(() => {
				if (verbose) logDebug(`debouncer fired for bridge: ${r.bridge.source} \u2192 [${r.bridge.consumers.map((c) => c.path).join(", ")}]`);
				pending.add(r);
				processPending();
			});
		...
		const configDebouncer = createDebouncer(async () => {
			...
			try {
				...
				} else logFail("Config reload failed — no valid bridges. Fix neutron.yml and save again.");
			} catch (err) {
		...
		function handleFileEvent(event, filePath) {
			...
			for (const r of resolved) if (matchesBridge(relPath, r)) {
				matched = true;
				if (verbose) logDebug(`  matched bridge: ${r.bridge.source} \u2192 [${r.bridge.consumers.map((c) => c.path).join(", ")}] (triggering debouncer)`);
	...
	setTimeout(() => {
		import("./outdated-cyo-M4TL.js").then(({ quickOutdatedCheck }) => quickOutdatedCheck(parsers)).then((msg) => {
...
//# sourceMappingURL=dev-Kp2THJ8p.js.map