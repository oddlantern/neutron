#!/usr/bin/env node
import { f as YELLOW, l as RED, u as RESET } from "./output-C8Qm-e8m.js";
import { t as MIDO_ROOT } from "./version-TAjFEXRG.js";
import { n as isRecord$1 } from "./bin.js";
...
async function readPackageJson(pkgPath, root) {
	...
	if (!isRecord$1(parsed)) throw new Error(`Expected object in ${manifestPath}`);
	return parsed;
}
/** Extract the scripts record from a parsed package.json */
function getScripts(manifest) {
	const scripts = manifest["scripts"];
	if (!isRecord$1(scripts)) return {};
	const result = {};
...
function hasDep(manifest, name, fields = DEFAULT_DEP_FIELDS) {
	for (const field of fields) {
		...
		if (isRecord$1(deps) && name in deps) return true;
	}
	return false;
}
/**
...
//#endregion
//#region src/plugins/builtin/design/types.ts
function isRecord(value) {
	return typeof value === "object" && value !== null && !Array.isArray(value);
}
function isThemedColor(value) {
	return isRecord(value) && typeof value["light"] === "string" && typeof value["dark"] === "string";
}
function isThemedNumber(value) {
...
*/
function resolveField(name, value) {
	...
	if (isThemedColor(value)) return {
		name,
		type: "color",
		value
	};
	if (isThemedNumber(value)) return {
		name,
		type: "number-themed",
		value
	};
	...
	for (const [name, ext] of Object.entries(extensions)) {
		...
		for (const [fieldName, fieldValue] of Object.entries(ext)) {
			if (fieldName === "_getter") {
				if (typeof fieldValue === "string") getter = fieldValue;
				continue;
			}
...
const HEADER$1 = `/* GENERATED BY MIDO — DO NOT EDIT */`;
/**
...
function generateCSS(tokens) {
	...
	if (tokens.spacing) {
		...
		for (const [key, value] of Object.entries(tokens.spacing)) lightVars.push(`  --spacing-${camelToKebab(key)}: ${value}px;`);
	}
	...
	if (tokens.iconSize) {
		...
		for (const [key, value] of Object.entries(tokens.iconSize)) lightVars.push(`  --icon-${camelToKebab(key)}: ${value}px;`);
	}
	lines.push(":root {");
...
function generateTS(tokens) {
	...
	if (tokens.spacing) {
		...
		for (const [key, value] of Object.entries(tokens.spacing)) lines.push(`  ${key}: ${value},`);
		lines.push("} as const;");
		lines.push("");
	}
	if (tokens.radius) {
		lines.push("export const DSRadius = {");
		for (const [key, value] of Object.entries(tokens.radius)) lines.push(`  ${key}: ${value},`);
		lines.push("} as const;");
		lines.push("");
	}
	if (tokens.iconSize) {
		lines.push("export const DSIconSize = {");
		for (const [key, value] of Object.entries(tokens.iconSize)) lines.push(`  ${key}: ${value},`);
		lines.push("} as const;");
		lines.push("");
	}
	if (tokens.spacing) lines.push("export type DSSpacingKey = keyof typeof DSSpacing;");
	if (tokens.radius) lines.push("export type DSRadiusKey = keyof typeof DSRadius;");
	if (tokens.iconSize) lines.push("export type DSIconSizeKey = keyof typeof DSIconSize;");
	lines.push("");
	return lines.join("\n");
}
//#endregion
...
function detectOxlintPlugins(pkg, root) {
	...
	try {
		...
		if (!isRecord$1(manifest)) return [...plugins];
...
/**
* Get the Dart lerp expression for a given extension field.
*/
function lerpExpr(fieldName, field) {
	switch (field.type) {
		case "color":
		case "color-static": return `Color.lerp(${fieldName}, other.${fieldName}, t)!`;
		case "number":
		case "number-themed": return `lerpDouble(${fieldName}, other.${fieldName}, t)!`;
		case "string": return `t < 0.5 ? ${fieldName} : other.${fieldName}`;
	}
}
/**
* Get the Dart type for a given extension field.
*/
function dartType(field) {
	switch (field.type) {
		case "color":
		case "color-static": return "Color";
		case "number":
		case "number-themed": return "double";
...
*/
function dartLightValue(field) {
	switch (field.type) {
		case "color": return hexToDartColor(field.value.light);
		case "color-static": return hexToDartColor(field.value);
		case "number": return `${field.value}`;
		case "number-themed": return `${field.value.light}`;
...
*/
function dartDarkValue(field) {
	switch (field.type) {
		case "color": return hexToDartColor(field.value.dark);
		case "color-static": return hexToDartColor(field.value);
		case "number": return `${field.value}`;
		case "number-themed": return `${field.value.dark}`;
		case "string": return `'${field.value}'`;
	}
}
...
function generateThemeExtensions(tokens) {
	const extensions = resolveExtensions(tokens.extensions);
	if (extensions.length === 0) return `${HEADER}\n\n// No extensions defined in tokens.json\n`;
	const lines = [
		HEADER,
		"",
		"import 'dart:ui';",
	...
	for (const ext of extensions) {
		...
		lines.push("  });");
		lines.push("");
		for (const field of fields) lines.push(`  final ${dartType(field)} ${field.name};`);
		lines.push("");
		lines.push(`  static const light = ${ext.name}(`);
		for (const field of fields) lines.push(`    ${field.name}: ${dartLightValue(field)},`);
		lines.push("  );");
		lines.push("");
		lines.push(`  static const dark = ${ext.name}(`);
		for (const field of fields) lines.push(`    ${field.name}: ${dartDarkValue(field)},`);
		lines.push("  );");
		lines.push("");
		lines.push("  @override");
		lines.push(`  ${ext.name} copyWith({`);
		for (const field of fields) lines.push(`    ${dartType(field)}? ${field.name},`);
		lines.push("  }) {");
		...
		lines.push(`    return ${ext.name}(`);
		for (const field of fields) lines.push(`      ${field.name}: ${lerpExpr(field.name, field)},`);
		lines.push("    );");
		lines.push("  }");
		lines.push("}");
		extIndex++;
		if (extIndex < extensions.length) lines.push("");
	}
	lines.push("");
	return lines.join("\n");
}
...
function generateConstants(tokens) {
	...
	if (tokens.spacing) {
		...
		for (const [key, value] of Object.entries(tokens.spacing)) lines.push(`  static const double ${key} = ${value};`);
		lines.push("}");
		lines.push("");
	}
	if (tokens.radius) {
		lines.push("abstract final class DSRadius {");
		for (const [key, value] of Object.entries(tokens.radius)) lines.push(`  static const double ${key} = ${value};`);
		lines.push("");
	...
	if (tokens.elevation) {
		...
		for (const [key, entry] of Object.entries(tokens.elevation)) lines.push(`  static const double ${key} = ${entry.dp};`);
		lines.push("}");
		lines.push("");
	}
	if (tokens.iconSize) {
		lines.push("abstract final class DSIconSize {");
		for (const [key, value] of Object.entries(tokens.iconSize)) lines.push(`  static const double ${key} = ${value};`);
		lines.push("}");
		lines.push("");
	}
	if (tokens.elevation) for (const mode of ["light", "dark"]) {
...
function generateTheme(tokens, packageName) {
	...
	for (const ext of extensions) lines.push(`  ${ext.name} get ${ext.getter} => Theme.of(this).extension<${ext.name}>()!;`);
	if (typo?.scale) {
		lines.push("");
	...
	if (typo?.fontFamily) {
		...
		if (body) lines.push(`      fontFamily: '${body}',`);
	}
	...
	if (typo?.scale) for (const [key, entry] of Object.entries(typo.scale)) {
		...
		const dartWeight = `FontWeight.w${typo.fontWeight[weightKey] ?? 400}`;
		let textStyleExpr;
		if (provider === "google_fonts") textStyleExpr = `GoogleFonts.${familyName.split(" ").map((w, i) => i === 0 ? w.toLowerCase() : w.charAt(0).toUpperCase() + w.slice(1)).join("")}(\n        fontSize: ${entry.size},\n        fontWeight: ${dartWeight},\n        color: scheme.onSurface,\n      )`;
		else if (provider === "asset") textStyleExpr = `TextStyle(\n        fontFamily: '${familyName}',\n        fontSize: ${entry.size},\n        fontWeight: ${dartWeight},\n        color: scheme.onSurface,\n      )`;
		else textStyleExpr = `TextStyle(\n        fontSize: ${entry.size},\n        fontWeight: ${dartWeight},\n        color: scheme.onSurface,\n      )`;
...
function isFlutterPackage(manifest) {
	...
	if (!isRecord$1(deps)) return false;
...
const hexColor = z.string().regex(/^#[0-9a-fA-F]{6}$/, "invalid hex color (expected #RRGGBB)");
...
function validateTokens(raw) {
	...
	for (const role of REQUIRED_COLOR_ROLES) if (!(role in data.color)) errors.push({
		path: `color.${role}`,
	...
	for (const role of OPTIONAL_COLOR_ROLES) if (!(role in data.color)) warnings.push({
		path: `color.${role}`,
...
*/
async function readTokens(artifactPath, root) {
...
const designPlugin = {
	...
	async detectBridge(artifact, root) {
		if (basename(artifact) !== "tokens.json") return false;
		try {
			return looksLikeTokens(await readTokens(artifact, root));
		} catch {
			return false;
		}
	},
	async exportArtifact(_source, artifact, root) {
		const start = performance.now();
		try {
			const result = validateTokens(await readTokens(artifact, root));
			if (!result.success) {
	...
	async generateDownstream(artifact, targets, root, context) {
		const validation = validateTokens(await readTokens(artifact, root));
...
async function detectEntryFile(packageDir) {
	try {
		...
		const parsed = JSON.parse(content);
		if (!isRecord$1(parsed)) throw new Error("Expected object");
...
async function detectFrameworkAdapter(pkgPath, root) {
	const { detectAdapter } = await import("./adapters-DWfm4iTY.js");
	try {
...
//# sourceMappingURL=registry-DuRpodRY.js.map